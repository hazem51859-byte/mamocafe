...
router.post('/', authenticateToken, requirePermission('manage_customers'), (req, res) => {
...
router.put('/:id', authenticateToken, requirePermission('manage_customers'), (req, res) => {
...
router.post('/:id/pay', authenticateToken, requirePermission('manage_customers'), (req, res) => {
  const id = req.params.id;
  ...
  const payTx = db.transaction(() => {
    ...
    db.prepare('UPDATE customers SET balance = balance - ? WHERE id = ?').run(amt, id);
...
router.delete('/:id', authenticateToken, requirePermission('manage_customers'), (req, res) => {
  const id = req.params.id;
  const salesCount = db.prepare('SELECT COUNT(*) as count FROM sales WHERE customer_id = ?').get(id).count;
  if (salesCount > 0) {
