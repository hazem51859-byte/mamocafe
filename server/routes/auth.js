...
router.post('/switch-user', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'حدث خطأ في تبديل المستخدم' });
  }
});
...
router.post('/change-password', authenticateToken, async (req, res) => {
  try {
    ...
    const user = await db.prepare('SELECT id, password_hash, username, full_name FROM users WHERE id = ?').get(req.user.id);
    if (!user) {
